...
const ProductAdmin = () => {
    ...
    const [formData, setFormData] = useState({
        title: '', author: '', price: '', discountPrice: '', stock: '',
        description: '', category: '', isbn: '', publisher: '', pages: '',
        year: '', imageUrl: '', featured: false
    });
    ...
    const handleDelete = async (productId) => {
        if (!window.confirm('Sigur doriți să ştergeţi acest produs?')) return;
        ...
        try {
            ...
            if (data.success) {
                ...
                alert(data.message || 'Eroare la ştergere');
            }
        } catch (error) {
                    ...
                    <div className="admin-modal-content">
                        ...
                        <form onSubmit={handleSaveProduct} className="admin-product-form">
                            <div className="admin-form-row">
                                <input type="text" placeholder="Titlu *" value={formData.title}
                                    onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                                    required className="admin-form-input" />
                                <input type="text" placeholder="Autor *" value={formData.author}
                                    onChange={(e) => setFormData({ ...formData, author: e.target.value })}
                                    required className="admin-form-input" />
                            </div>

                            <div className="admin-form-row">
                                <input type="number" step="0.01" placeholder="Preț *"
                                    value={formData.price} onChange={(e) => setFormData({ ...formData, price: e.target.value })}
                                    required className="admin-form-input" />
                                <input type="number" step="0.01" placeholder="Preț redus"
                                    value={formData.discountPrice} onChange={(e) => setFormData({ ...formData, discountPrice: e.target.value })}
                                    className="admin-form-input" />
                            </div>

                            <div className="admin-form-row">
                                <input type="number" placeholder="Stoc *" value={formData.stock}
                                    onChange={(e) => setFormData({ ...formData, stock: e.target.value })}
                                    required className="admin-form-input" />
                            </div>

                            <textarea placeholder="Descriere" value={formData.description}
                                onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                                rows="3" className="admin-form-textarea" />

                            <div className="admin-form-row">
                                <input type="text" placeholder="Categorie" value={formData.category}
                            ...
                            <div className="admin-form-row">
                                <input type="text" placeholder="Editura" value={formData.publisher}
                                    onChange={(e) => setFormData({ ...formData, publisher: e.target.value })}
                                    className="admin-form-input" />
                                <input type="text" placeholder="An apariție" value={formData.year}
                                    onChange={(e) => setFormData({ ...formData, year: e.target.value })}
                                    className="admin-form-input" />        
                            </div>
                            <div className='admin-form-row'>
                                 <input type="number" step="0.01" placeholder="Pagini *"
                                    value={formData.pages} onChange={(e) => setFormData({ ...formData, pages: e.target.value })}
                                    required className="admin-form-input" />
                                <input type="text" placeholder="URL imagine" value={formData.imageUrl}
                        ...
                        <div key={product.id} className={`admin-product-card ${!product.isActive ? 'admin-product-inactive' : ''}`}>
                                    ...
                                    {product.discountPrice ? (
                                        <>
                                            ...
                                            <span className="admin-product-discount-price">
                                                {product.discountPrice} RON</span>
                            ...
                            <div className="admin-product-actions">
                                <button onClick={() => handleEdit(product)}
                                    className="admin-btn admin-btn-edit">Editare</button>
                                ...
                                <button onClick={() => handleDelete(product.id)}
                                    className="admin-btn admin-btn-danger">Şterge</button>
                            </div>
